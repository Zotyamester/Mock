...
class ConsultationForm(FlaskForm):
    lastname = StringField('Vezetéknév', validators=[DataRequired(), Regexp('^[A-Z][a-z]*$'), Length(min=1, max=64)])
    firstname = StringField('Keresztnév', validators=[DataRequired(), Regexp('^[A-Z][a-z]*$'), Length(min=1, max=64)])
    age = IntegerField('Életkor', validators=[NumberRange(min=18), InputRequired()])
    email = StringField('E-mail', validators=[DataRequired(), Length(min=5, max=320), Email()])
    postcode = StringField('Irányítószám', validators=[InputRequired(), Length(min=4, max=4)])
    ...
    def validate_postcode(self, postcode):
        if not postcode.data.isnumeric():
            raise ValidationError('Érvénytelen karakterek')
